...
import feign.remoting.connection.handler.HeartbeatHandler;
...
import feign.remoting.connection.manager.ReconnectManager;
import feign.remoting.connection.processor.ConnectionEventProcessor;
import feign.remoting.connection.strategy.ConnectionMonitorStrategy;
import feign.remoting.connection.strategy.ConnectionSelectStrategy;
import feign.remoting.connection.strategy.DefaultConnectionMonitor;
import feign.remoting.connection.strategy.RandomSelectStrategy;
import feign.remoting.connection.strategy.ScheduledDisconnectStrategy;
...
import feign.remoting.invoke.RpcClientRemoting;
...
import feign.remoting.util.RpcTaskScanner;
import feign.remoting.connection.factory.DefaultConnectionFactory;
 ...
 */
public class ShuttleClient extends AbstractConfigurableInstance {

    /** logger */
    private static final Logger                         logger                   = LoggerFactory.getLogger("ShuttleClient");
    ...
    private ConnectionFactory                           connectionFactory        = new DefaultConnectionFactory(
    																					...
    																					new DispatcherHandler(userProcessors),
    																					this);
...
//    }
    ...
    public void init() {
        ...
        //====================================================================
        //核心的初始化方法，在这个方法中包括了以下工作：
        //====================================================================
        //1、将ConnectionEventHandler的ConnectionManager设置为当前DefaultConnectionManager实例
        //2、将connectionEventLinstener的设置到connectionEventHandler中
        //3、调用DefaultConnectionFactory的init方法，在该方法中会创建BootStrap对象，
        //   并设置ConnectionEventHandler、DispatcherHandler等参数
        ...
        //由于connectionManager实现了Scanable接口，所以将connectionManager加入到定时扫描的线程池中
        //taskScanner线程池，每隔10秒钟启动一次线程池扫描任务
        //this.taskScanner.add(this.connectionManager);
        //this.taskScanner.start();

        //判断是否开启了连接监控，即：定时对连接进行关闭
    ...
    public Object invokeSync(final Connection conn, final Object request,
                             final InvokeContext invokeContext, final int timeoutMillis) throws RemotingException,
                                                                                        InterruptedException {
     ...
     * <p>
     * Notice: This api should be called before {@link ShuttleClient#init()}
     */
